...
- `desktop` liberator manages GNOME dock favourites from `instances/<host>/desktop/favorite-apps` (one `.desktop` id per line, order = dock order). A file that is present but yields no entries is refused with a warning rather than applied -- applying it empties the dock, and that is far more likely to be a malformed file or a glob that matched nothing than a deliberate bare dock. The dock silently outranks every default-terminal setting: a sleeve with alacritty set as default by both gsettings and `/etc/alternatives` still launched gnome-terminal, because both were pinned and the icon was what got clicked
...
- `editors maintain` warns when straight.el has left dangling links in the ACTIVE build tree. straight adds and updates build-dir entries but never PRUNES ones whose source vanished upstream, so a renamed or dropped file leaves a link pointing at nothing and Emacs dies at boot with a bare `(file-missing ...)`. Runs after `doom sync -u`, which is the only thing that causes it; not in `_check`, where a 6000-entry scan costs ~5s (`find -L` makes it ~60ms)
...
- `intent` liberator installs Intent from Homebrew (`brew install matthewsinclair/intent/intent`) instead of linking a source checkout's dispatcher into `~/bin`. Its check fails, naming the path, when `intent` on PATH is not Homebrew's, and when the pre-commit gate's home is missing, names an install that is gone, or names another install. On gyges the old check printed ok over a link to Intent 2.6.0 while a login shell ran Homebrew's 3.2.0. `MOLT_INTENT_HOME`, the `~/bin/intent` link and `molt git` over the Intent checkout are gone. Disabled on rhadamanth, where Intent is developed (ST0004)
...
- Pipelines ending in `grep -q` converted to capture-then-match at all six sites (`bootstrap.sh` git URL probe, `molt_foreign_home_paths`, doctor's GitHub auth check, `system` sudo group, `backup` locked-daemon probe, `editors` ImageMagick probe). Under `pipefail` an early-exiting `grep -q` can SIGPIPE its writer and turn a found match into a failure. `molt_foreign_home_paths` could lose already: a foreign home path near the top of a large file went unreported. Its conversion tests for an empty capture explicitly, because `grep -qv` on an empty herestring answers true. Intent's `pipefail_sigpipe_check.sh` guard was not ported
- `alacritty` never flagged a template-form config. `_alacritty_config_wrong` passed `molt_config_stale` the absolute `.tmpl` path, which then looked for `alacritty.toml.tmpl.tmpl` and answered "not stale". On kovacs `~/.config/alacritty/alacritty.toml` was a dangling link from Molt-matts `573e401` (8 Sep) onward, while resleeve reported alacritty ok. It now passes the repo-relative source, and `molt_config_stale` rejects an absolute or `.tmpl` source loudly and answers stale instead of passing. `test/liberators/alacritty.bats` is the liberator's first test file (7 tests)
- `git_check` and `git_verify` tested only `~/.gitconfig`, while `git_install` also links `~/.gitignore_global` and every `gitconfig_*` identity include. On rhadamanth and gyges `~/.gitignore_global` was a stale regular file, and every run reported git ok until a repo move broke `~/.gitconfig` and install finally ran. All three now read one link list (`_git_links`), and check and verify require each link to point at its source in the user repo, not just resolve. `git_verify` fails when the user repo cannot be found instead of checking one link and passing
