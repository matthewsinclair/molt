...
- `molt doctor` check (12th): warns when `core.ignorecase=true` in the managed repos. A case-only rename is then not committed, so a case-sensitive sleeve pulls the old spelling and every symlink to it dangles -- the mechanism that hid the repo rename
